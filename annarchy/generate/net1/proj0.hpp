...
/////////////////////////////////////////////////////////////////////////////
// proj0: hippocampus -> nacc_shell_d1 with target exc
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct0 : LILMatrix<int, int> {
    ProjStruct0() : LILMatrix<int, int>( 24, 400) {
    }
        ...
        if(_transmission && _update && pop3._active && ( (t - _update_offset)%_update_period == 0L) ){
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    ...
                    double _alpha = (-alpha[i][j] + positive(pop3.mp[rk_post] - regularization_threshold[i][j]))/tau_alpha[i][j];
                    ...
                    dopa_sum[i][j] = 2.0*(pop3._sum_dopa[rk_post] - baseline_dopa[i][j]);


                    // trace = pos(post.r - mean(post.r) - threshold_post) * (pre.r - mean(pre.r) - threshold_pre)
                    trace[i][j] = (-pop0._mean_r + pop0.r[rk_pre] - threshold_pre[i][j])*positive(-pop3._mean_r + pop3.r[rk_post] - threshold_post[i][j]);
                    ...
                    delta[i][j] = -alpha[i][j]*positive(-pop3._mean_r + pop3.r[rk_post] - threshold_post[i][j])*positive(-pop3._mean_r + pop3.r[rk_post] - threshold_post[i][j]) + dopa_mod[i][j]*trace[i][j];
