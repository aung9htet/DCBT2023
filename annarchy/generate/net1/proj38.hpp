...
/////////////////////////////////////////////////////////////////////////////
// proj38: striatum_d1_dorsomedial -> gpi_dorsomedial with target inh
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct38 : LILMatrix<int, int> {
    ProjStruct38() : LILMatrix<int, int>( 6, 48) {
    }
    ...
    // Local parameter negterm
    std::vector< std::vector<double > > negterm;
    ...
    bool init_attributes() {
        ...
        // Local parameter negterm
        negterm = init_matrix_variable<double>(static_cast<double>(0.0));
    ...
    #ifdef __AVX__
        if (_transmission && pop19._active) {
            int _s, _stop;
            double _tmp_sum[4];
            double* __restrict__ _pre_r = pop16.r.data();
    ...
    void update_synapse() {
        ...
        // Check periodicity
        if(_transmission && _update && pop19._active && ( (t - _update_offset)%_update_period == 0L) ){
            // Global variables
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    ...
                    double _alpha = (-alpha[i][j] + positive(-pop19.mp[rk_post] - regularization_threshold[i][j]))/tau_alpha[i][j];
                    ...
                    dopa_sum[i][j] = 2.0*(pop19._sum_dopa[rk_post] - baseline_dopa[i][j]);


                    // trace = pos(pre.trace - mean(pre.trace) - threshold_pre) * (mean(post.r) - post.r - threshold_post)
                    trace[i][j] = (pop19._mean_r - pop19.r[rk_post] - threshold_post[i][j])*positive(-pop16._mean_trace + pop16.trace[rk_pre] - threshold_pre[i][j]);


                    // aux = if (trace > 0): negterm else: 0
                    aux[i][j] = (trace[i][j] > 0 ? negterm[i][j] : 0);


                    // dopa_mod = if (DA_type * dopa_sum > 0): K_burst * dopa_sum else: aux * DA_type * K_dip * dopa_sum
                    dopa_mod[i][j] = (DA_type[i][j]*dopa_sum[i][j] > 0 ? K_burst[i][j]*dopa_sum[i][j] : DA_type[i][j]*K_dip[i][j]*aux[i][j]*dopa_sum[i][j]);
    ...
    std::vector<std::vector<double>> get_local_attribute_all_double(std::string name) {
    #ifdef _DEBUG
        std::cout << "ProjStruct38::get_local_attribute_all_double(name = "<<name<<")" << std::endl;
    #endif
        ...
        // Local parameter negterm
        if ( name.compare("negterm") == 0 ) {

            return get_matrix_variable_all<double>(negterm);
        }
    ...
    std::vector<double> get_local_attribute_row_double(std::string name, int rk_post) {
    #ifdef _DEBUG
        std::cout << "ProjStruct38::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<")" << std::endl;
    #endif
        ...
        if ( name.compare("negterm") == 0 ) {
            ...
            return get_matrix_variable_row<double>(negterm, rk_post);
        }
    ...
    double get_local_attribute_double(std::string name, int rk_post, int rk_pre) {
    #ifdef _DEBUG
        std::cout << "ProjStruct38::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<")" << std::endl;
    #endif
        ...
        if ( name.compare("negterm") == 0 ) {
            ...
            return get_matrix_variable<double>(negterm, rk_post, rk_pre);
        }
    ...
    void set_local_attribute_all_double(std::string name, std::vector<std::vector<double>> value) {
        ...
        // Local parameter negterm
        if ( name.compare("negterm") == 0 ) {
            update_matrix_variable_all<double>(negterm, value);
    ...
    void set_local_attribute_row_double(std::string name, int rk_post, std::vector<double> value) {
        ...
        // Local parameter negterm
        if ( name.compare("negterm") == 0 ) {
            update_matrix_variable_row<double>(negterm, rk_post, value);
    ...
    void set_local_attribute_double(std::string name, int rk_post, int rk_pre, double value) {
        ...
        // Local parameter negterm
        if ( name.compare("negterm") == 0 ) {
            update_matrix_variable<double>(negterm, rk_post, rk_pre, value);
    ...
    long int size_in_bytes() {
        ...
        // Local parameter negterm
        size_in_bytes += sizeof(std::vector<std::vector<double>>);
        size_in_bytes += sizeof(std::vector<double>) * negterm.capacity();
        for(auto it = negterm.cbegin(); it != negterm.cend(); it++)
            size_in_bytes += (it->capacity()) * sizeof(double);
    ...
    void clear() {
        ...
        // negterm
        for (auto it = negterm.begin(); it != negterm.end(); it++) {
            it->clear();
            it->shrink_to_fit();
        };
        negterm.clear();
        negterm.shrink_to_fit();
