...
/////////////////////////////////////////////////////////////////////////////
// proj2: nacc_shell_d1 -> vp_direct with target inh
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct2 : LILMatrix<int, int> {
    ProjStruct2() : LILMatrix<int, int>( 6, 24) {
    }
    ...
    void compute_psp() {
        ...
        if (_transmission && pop6._active) {
            int _s, _stop;
            double _tmp_sum[4];
            double* __restrict__ _pre_r = pop3.r.data();
        ...
        if(_transmission && _update && pop6._active && ( (t - _update_offset)%_update_period == 0L) ){
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    ...
                    trace[i][j] = (pop6._mean_r - pop6.r[rk_post] - threshold_post[i][j])*positive(-pop3._mean_r + pop3.r[rk_pre] - threshold_pre[i][j]);
