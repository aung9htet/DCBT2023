...
///////////////////////////////////////////////////////////////
// Main Structure for the population of id 21 (substantia_nigra_pars_compacta)
///////////////////////////////////////////////////////////////
struct PopStruct21{
    ...
    #ifdef _DEBUG
        std::cout << "PopStruct21::init_population(size="<<this->size<<") - this = " << this << std::endl;
    #endif
        _active = true;
    ...
    void update() {
        ...
        if( _active ) {
            ...
            for(int i = 0; i < size; i++){
                ...
                aux[i] = (firing[i] > 0 ? ex_in[i]*(baseline[i] + positive(-baseline[i] - s_inh[i] + 1.0)) + (1 - ex_in[i])*(_sum_inh[i]*(-factor_inh[i]) + baseline[i]) : baseline[i]);
    ...
    long int size_in_bytes() {
        ...
        size_in_bytes += sizeof(std::vector<double>) + sizeof(double) * firing.capacity();	// firing
        size_in_bytes += sizeof(std::vector<double>) + sizeof(double) * inhibition.capacity();	// inhibition
        size_in_bytes += sizeof(std::vector<double>) + sizeof(double) * baseline.capacity();	// baseline
        size_in_bytes += sizeof(std::vector<double>) + sizeof(double) * exc_threshold.capacity();	// exc_threshold
        size_in_bytes += sizeof(std::vector<double>) + sizeof(double) * factor_inh.capacity();	// factor_inh
        // Variables
        size_in_bytes += sizeof(std::vector<double>) + sizeof(double) * ex_in.capacity();	// ex_in
        size_in_bytes += sizeof(std::vector<double>) + sizeof(double) * s_inh.capacity();	// s_inh
        size_in_bytes += sizeof(std::vector<double>) + sizeof(double) * aux.capacity();	// aux
