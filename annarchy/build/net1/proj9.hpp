...
/////////////////////////////////////////////////////////////////////////////
// proj9: hippocampus -> nacc_shell_d2 with target exc
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct9 : LILMatrix<int, int> {
    ProjStruct9() : LILMatrix<int, int>( 24, 400) {
    }
    ...
    void compute_psp() {
        ...
        if (_transmission && pop4._active) {
            int _s, _stop;
            double _tmp_sum[4];
            double* __restrict__ _pre_r = pop0.r.data();
    ...
    void update_synapse() {
        ...
        // Check periodicity
        if(_transmission && _update && pop4._active && ( (t - _update_offset)%_update_period == 0L) ){
            // Global variables
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    ...
                    double _alpha = (-alpha[i][j] + positive(pop4.mp[rk_post] - regularization_threshold[i][j]))/tau_alpha[i][j];
                    ...
                    dopa_sum[i][j] = 2.0*(pop4._sum_dopa[rk_post] - baseline_dopa[i][j]);


                    // trace = pos(post.r - mean(post.r) - threshold_post) * (pre.r - mean(pre.r) - threshold_pre)
                    trace[i][j] = (-pop0._mean_r + pop0.r[rk_pre] - threshold_pre[i][j])*positive(-pop4._mean_r + pop4.r[rk_post] - threshold_post[i][j]);
                    ...
                    delta[i][j] = -alpha[i][j]*positive(-pop4._mean_r + pop4.r[rk_post] - threshold_post[i][j])*positive(-pop4._mean_r + pop4.r[rk_post] - threshold_post[i][j]) + dopa_mod[i][j]*trace[i][j];
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct9::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<")" << std::endl;
    #endif
