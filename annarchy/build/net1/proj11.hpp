...
/////////////////////////////////////////////////////////////////////////////
// proj11: nacc_shell_d2 -> vp_indirect with target inh
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct11 : LILMatrix<int, int> {
    ProjStruct11() : LILMatrix<int, int>( 6, 24) {
    }
    ...
    void compute_psp() {
        ...
        if (_transmission && pop7._active) {
            int _s, _stop;
            double _tmp_sum[4];
            double* __restrict__ _pre_r = pop4.r.data();
    ...
    void update_synapse() {
        ...
        // Check periodicity
        if(_transmission && _update && pop7._active && ( (t - _update_offset)%_update_period == 0L) ){
            // Global variables
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    ...
                    double _alpha = (-alpha[i][j] + positive(-pop7.mp[rk_post] - regularization_threshold[i][j]))/tau_alpha[i][j];
                    ...
                    dopa_sum[i][j] = 2.0*(pop7._sum_dopa[rk_post] - baseline_dopa[i][j]);


                    // trace = pos(pre.r - mean(pre.r) - threshold_pre) * (mean(post.r) - post.r - threshold_post)
                    trace[i][j] = (pop7._mean_r - pop7.r[rk_post] - threshold_post[i][j])*positive(-pop4._mean_r + pop4.r[rk_pre] - threshold_pre[i][j]);
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct11::get_local_attribute_all_double(name = "<<name<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct11::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct11::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<")" << std::endl;
    #endif
