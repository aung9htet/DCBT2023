...
/////////////////////////////////////////////////////////////////////////////
// proj13: medial_pfc -> stn_ventral with target exc
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct13 : LILMatrix<int, int> {
    ProjStruct13() : LILMatrix<int, int>( 24, 6) {
    }
    ...
    void update_synapse() {
        ...
        // Check periodicity
        if(_transmission && _update && pop5._active && ( (t - _update_offset)%_update_period == 0L) ){
            // Global variables
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    ...
                    double _alpha = (-alpha[i][j] + positive(pop5.mp[rk_post] - regularization_threshold[i][j]))/tau_alpha[i][j];
                    ...
                    dopa_sum[i][j] = 2.0*(pop5._sum_dopa[rk_post] - baseline_dopa[i][j]);


                    // trace = pos(post.r - mean(post.r) - threshold_post) * (pre.r - mean(pre.r) - threshold_pre)
                    trace[i][j] = (-pop12._mean_r + pop12.r[rk_pre] - threshold_pre[i][j])*positive(-pop5._mean_r + pop5.r[rk_post] - threshold_post[i][j]);
                    ...
                    delta[i][j] = -alpha[i][j]*positive(-pop5._mean_r + pop5.r[rk_post] - threshold_post[i][j])*positive(-pop5._mean_r + pop5.r[rk_post] - threshold_post[i][j]) + dopa_mod[i][j]*trace[i][j];
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct13::get_local_attribute_all_double(name = "<<name<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct13::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct13::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<")" << std::endl;
    #endif
