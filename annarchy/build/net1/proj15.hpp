...
/////////////////////////////////////////////////////////////////////////////
// proj15: stn_ventral -> vp_direct with target exc
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct15 : LILMatrix<int, int> {
    ProjStruct15() : LILMatrix<int, int>( 6, 24) {
    }
    ...
    void compute_psp() {
        ...
        if (_transmission && pop6._active) {
            int _s, _stop;
            double _tmp_sum[4];
            double* __restrict__ _pre_r = pop5.r.data();
    ...
    void update_synapse() {
        ...
        // Check periodicity
        if(_transmission && _update && pop6._active && ( (t - _update_offset)%_update_period == 0L) ){
            // Global variables
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    ...
                    double _alpha = (-alpha[i][j] + positive(pop6.mp[rk_post] - regularization_threshold[i][j]))/tau_alpha[i][j];
                    ...
                    dopa_sum[i][j] = 2.0*(pop6._sum_dopa[rk_post] - baseline_dopa[i][j]);


                    // trace = pos(pre.r - mean(pre.r) - threshold_pre) * (post.trace - mean(post.trace) - threshold_post)
                    trace[i][j] = (-pop6._mean_trace + pop6.trace[rk_post] - threshold_post[i][j])*positive(-pop5._mean_r + pop5.r[rk_pre] - threshold_pre[i][j]);


                    // aux = if (trace < 0.0): 1 else: 0
                    aux[i][j] = (trace[i][j] < 0.0 ? 1 : 0);


                    // dopa_mod = if (dopa_sum > 0): K_burst * dopa_sum else: K_dip * dopa_sum * aux
                    dopa_mod[i][j] = (dopa_sum[i][j] > 0 ? K_burst[i][j]*dopa_sum[i][j] : K_dip[i][j]*aux[i][j]*dopa_sum[i][j]);
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct15::get_local_attribute_all_double(name = "<<name<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct15::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct15::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<")" << std::endl;
    #endif
