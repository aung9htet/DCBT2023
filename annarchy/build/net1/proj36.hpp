...
/////////////////////////////////////////////////////////////////////////////
// proj36: medial_pfc -> striatum_d1_dorsomedial with target exc
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct36 : LILMatrix<int, int> {
    ProjStruct36() : LILMatrix<int, int>( 48, 6) {
    }
    ...
    void compute_psp() {
        ...
        if (_transmission && pop16._active) {
            int _s, _stop;
            double _tmp_sum[4];
            double* __restrict__ _pre_r = pop12.r.data();
    ...
    void update_synapse() {
        ...
        // Check periodicity
        if(_transmission && _update && pop16._active && ( (t - _update_offset)%_update_period == 0L) ){
            // Global variables
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    ...
                    double _alpha = (-alpha[i][j] + positive(pop16.mp[rk_post] - regularization_threshold[i][j]))/tau_alpha[i][j];
                    ...
                    dopa_sum[i][j] = 2.0*(pop16._sum_dopa[rk_post] - baseline_dopa[i][j]);


                    // trace = pos(post.trace - mean(post.trace) - threshold_post) * (pre.r - mean(pre.r) -threshold_pre)
                    trace[i][j] = (-pop12._mean_r + pop12.r[rk_pre] - threshold_pre[i][j])*positive(-pop16._mean_trace + pop16.trace[rk_post] - threshold_post[i][j]);
                    ...
                    delta[i][j] = -alpha[i][j]*positive(-pop16._mean_r + pop16.r[rk_post] - threshold_post[i][j])*positive(-pop16._mean_r + pop16.r[rk_post] - threshold_post[i][j]) + dopa_mod[i][j]*trace[i][j];
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct36::get_local_attribute_all_double(name = "<<name<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct36::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct36::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<")" << std::endl;
    #endif
