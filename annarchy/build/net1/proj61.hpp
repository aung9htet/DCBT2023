...
/////////////////////////////////////////////////////////////////////////////
// proj61: striatum_d1_dorsomedial -> substantia_nigra_pars_compacta with target inh
/////////////////////////////////////////////////////////////////////////////
struct ProjStruct61 : LILMatrix<int, int> {
    ProjStruct61() : LILMatrix<int, int>( 6, 48) {
    }
    ...
    void update_synapse() {
        ...
        // Check periodicity
        if(_transmission && _update && pop21._active && ( (t - _update_offset)%_update_period == 0L) ){
            // Global variables
            ...
            for (int i = 0; i < post_rank.size(); i++) {
                ...
                for (int j = 0; j < pre_rank[i].size(); j++) {
                    ...
                    aux[i][j] = (pop21._sum_exc[rk_post] > 0.001 ? 1.0 : 2.5);


                    // delta = aux * (post.r - baseline_dopa) * pos(pre.r - mean(pre.r))
                    delta[i][j] = aux[i][j]*(pop21.r[rk_post] - baseline_dopa[i][j])*positive(-pop16._mean_r + pop16.r[rk_pre]);
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct61::get_local_attribute_all_double(name = "<<name<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct61::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<")" << std::endl;
    #endif
    ...
    #ifdef _DEBUG
        std::cout << "ProjStruct61::get_local_attribute_row_double(name = "<<name<<", rk_post = "<<rk_post<<", rk_pre = "<<rk_pre<<")" << std::endl;
    #endif
